function [otime,elat,elon,edep,strike,dip,rake,M,Mw,eid,capmod,capdep,rms,vr,Pwin,Swin,Nstn,Pstn,Sstn,...
    stnm,lampPV,lampPR,lampSV,lampSR,lampSH,corrPV,corrPR,corrSV,corrSR,corrSH,pPol,ePol,shiftSavg,shiftPavg] = read_capout(filename)
...
[~,eid,~,ctag,~,strike,dip,rake,~,Mw,~,rms,X,~,delta,~,gamma,~,vr,~,dat2,~,pol_wt] = ...
    strread(lines{1},'%s%s%s%s%s%f%f%f%s%f%s%f%f%s%f%s%f%s%f%s%f%s%f');
...
for ii=1:Nstn
    ...
    [stnm(ii),~,wtPV(ii),errPV(ii),corrPV(ii),shiftPV(ii),lampPV(ii),DampPV(ii),SampPV(ii),...
        ...
        shiftSavg(ii) = (shiftSV(ii) + shiftSR(ii) + shiftSH(ii))/(3);
        shiftPavg(ii) = (shiftPV(ii) + shiftPR(ii))/(2);
